...
export default function Act({ 
    ...
    const convertUTCToMSK = (timeString) => {
        if (!timeString) return '';
        const [hours, minutes, seconds] = timeString.split(':').map(Number);
        const mskHours = (hours + 4) % 24;
        return `${mskHours.toString().padStart(2, '0')}:${minutes.toString().padStart(2, '0')}`;
    };
    ...
    const buildEventDescription = () => {
        ...
        if (responseReport) {
            parts.push(`Комментарий бригадира: ${responseReport}`);
        }
    ...
    const localTime = convertUTCToMSK(accidentTime);

    const [form, setForm] = useState({
        org: 'ООО «Саратовский промышленный терминал»',
        number: responseId,
        city: 'Саратов',
        day: currentDate.day,
    ...
    useEffect(() => {
        const updatedLocalTime = convertUTCToMSK(accidentTime);

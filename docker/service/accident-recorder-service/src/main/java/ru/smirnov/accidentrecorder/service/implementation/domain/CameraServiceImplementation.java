...
public class CameraServiceImplementation implements CameraService {
    ...
    @Autowired
    public CameraServiceImplementation(
    ...
    public Long createCamera(CameraCreationRequest dto) {
        Area area = this.areaPreconditionService.safelyGetById(dto.getAreaId());
    ...
    public List<CameraResponse> getCameras() {
        return this.cameraRepository.findAll().stream()
                .map(this.cameraMapper::cameraEntityToCameraResponse)
                .sorted(Comparator.comparing(CameraResponse::getAreaName))
                .toList();
    }

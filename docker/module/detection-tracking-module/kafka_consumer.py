...
from worker_pool import executor
...
def process_video_wrapper(local_path: str, key: str):
    ...
    try:
        ...
        logger.info(f"[WORKER] start processing: {key}")

        process_video(local_path, key)

        logger.info(f"[WORKER] done processing: {key}")

    except Exception as e:
        logger.exception(f"[WORKER] processing error for {key}: {e}")
...
def handle_message(message):
    try:
        ...
        logger.info(f"[KAFKA] received: {key}")
        ...
        logger.info(f"[KAFKA] submit task: {key}")
...
def start_consumer():
    ...
    while True:
        ...
        try:
            ...
            logger.info(
                f"[KAFKA] Attempting to connect to Kafka "
                f"(attempt {attempt})"
            )
